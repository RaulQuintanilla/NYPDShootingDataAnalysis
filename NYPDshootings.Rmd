...
```
## Introduction
This dataset contains information on shooting related deaths in New York dating back to 2006.
...
ggplot(death_counts, aes(year,Deaths, fill = Deaths)) + 
  geom_bar(stat = "identity") + coord_cartesian(ylim = c(300,375)) + 
...
```{r}
#split date into more useable format
...
# maximum deaths, year it occured, and precinct
...
By applying a linear model to the data for Precinct 75, we can see a decreasing trend in deaths over the course of the range of dates. In terms of bias, it is possible for there to un unreported or undiscovered deaths at the time at data is collected.
